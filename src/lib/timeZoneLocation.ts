...
function cityFor(timeZone: string): string {
...
/** A compact flag-and-city treatment for the read-only profile summary. */
export function timeZoneLocation(timeZone: string): TimeZoneLocation {
  const city = cityFor(timeZone)
  const countryCode = TIME_ZONE_COUNTRIES[timeZone]
  if (!countryCode) {
    return {
      countryCode: null,
      city,
      accessibleLabel: city === 'UTC' ? 'UTC time zone' : `${city} time zone`,
    }
  }
  ...
  return {
    countryCode,
    city,
    accessibleLabel: `${country}, ${city} time zone`,
  }
}
