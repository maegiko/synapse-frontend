...
/**
 * A primary task card beside a supporting aside. The aside carries no surface of
 * its own, so it reads as guidance rather than a competing card.
 */
...
/** Compact, background-free metadata used throughout the product UI. */
export const countPill =
  'inline-flex min-h-5 items-center text-xs font-normal text-text-muted tabular-nums'
...
/**
 * Gold rather than the accent purple, so a pin never reads as the link
 * affordance beside it, and `warning-solid` because it stays legible on both
 * `surface` and `control` in either theme.
 */
