...
/**
 * The one sentence this page is allowed to say about a request that reached the
 * backend. Unknown address, unverified account, live account, and a failed
 * email send all answer `204`, and a `429` counts unknown addresses too, so
 * anything more specific would leak whether an account exists.
 */
const GENERIC_CONFIRMATION = 'If an account exists for that address, we sent a password-reset link.'

/**
 * Where a forgotten password starts: a public form that posts one address to
 * `POST /api/auth/password/forgot`.
 *
 * Public rather than guest-only, because the reset page it leads to is public
 * too and somebody with a stale link needs to get back here whatever session
 * the browser happens to be holding.
 *
 * Every outcome that reached the backend ends in the same confirmation. The
 * only remedy the UI can offer is sending again, and the copy never promises
 * that an email is actually on its way.
 */
export function ForgotPasswordPage() {
  ...
  /** True once a request reached the backend, whether it answered 204 or 429. */
  const [requested, setRequested] = useState(false)
  const cooldown = useCooldown()

  // The confirmation replaces the form the visitor just submitted, so focus
  // moves to it rather than being left on a control that no longer exists.
  ...
  async function send(address: string) {
    ...
    try {
      ...
      // A 400 is about the text that was typed, not about any account, so it
      // belongs on the field and the form comes back to be corrected.
      if (isStatus(error, 400)) {
        setRequested(false)
        setFieldError('Enter a valid email address.')
        return
      }
      // A rate limit still lands on the confirmation, with the same wording: a
      // 429 counts addresses that have no account, so treating it as a special
      // case would reveal exactly what the 204 hides. Only the wait is new.
      if (isStatus(error, 429)) {
        cooldown.start(retryAfterSeconds(error))
        setRequested(true)
        return
      }
      // Nothing reached the backend (offline, or the server itself failed), so
      // this one is not about the account either.
